...
def calculate_shape_noise(shear_catalogs, nside, 
    ...
    w2_sigma2_map = np.zeros(hp.nside2npix(nside), dtype=np.float32)
    ...
    for catalog in shear_catalogs:
        ...
        w2_sigma2_map += np.bincount(pixel_idx, weights=w**2*(e1**2 + e2**2)/2,
                                    minlength=n_pix)
    mean_w2_sigma2 = w2_sigma2_map.sum() / n_pix
    noise_bias = hp.nside2pixarea(nside) * mean_w2_sigma2
    return noise_bias


def create_shear_healpix_triplet(shear_catalogs, shear_healpix_path, nside, 
    ...
    s2w2_map = np.zeros(hp.nside2npix(nside), dtype=np.float32)
    
    out_filenames = {"triplet" :        os.path.join(shear_healpix_path, "triplet.fits"),
                     "triplet_random":os.path.join(shear_healpix_path, "triplet_random.fits"),
    ...
    for ic, catalog in enumerate(shear_catalogs):
        if verbose: print("Reading {}.".format(catalog))
        ra, dec, e1, e2, w, m = prepare_catalog(catalog, column_names, 
                                                c_correction, m_correction, 
                                                z_min, z_max, selections, hdu_idx)
        ...
        if shear_randoms:
            ...
            e = np.sqrt(e1**2 + e2**2)
            e1_random = np.cos(2.0*alpha)*e
            e2_random = np.sin(2.0*alpha)*e
            if ic == 0:
                e1_r_map = np.zeros(hp.nside2npix(nside), dtype=np.float32)
                e2_r_map = np.zeros(hp.nside2npix(nside), dtype=np.float32)
            np.add.at(e1_r_map, pixel_idx, e1_random*w)
            np.add.at(e2_r_map, pixel_idx, e2_random*w)
        ...
        np.add.at(s2w2_map, pixel_idx, w**2*(e1**2 + e2**2)/2)
    ...
    e1_r_map[weight_mask] /= K_map[weight_mask]
    e2_r_map[weight_mask] /= K_map[weight_mask]
    ...
    e1_r_map[~weight_mask] = masked_value
    e2_r_map[~weight_mask] = masked_value
    ...
    noise_bias = np.mean(s2w2_map) * hp.nside2pixarea(nside)
    print(noise_bias)
    # Writing to disk

    if shear_randoms:
        hp.write_map(out_filenames["triplet_random"], [rho_map, e1_r_map, e2_r_map], partial=False, coord=coord, fits_IDL=False, overwrite=True)
